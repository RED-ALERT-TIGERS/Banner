...
mv $HOME/TIGERS/files/chat.sh /data/data/com.termux/files/usr/bin/chat
...
if [ -f "$dx" ]; then
	...
	cp $HOME/TIGERS/files/font.ttf "$ds"
fi
...
if [ -f "$simu" ]; then
	...
	cp $HOME/TIGERS/files/colors.properties "$ds"
fi
cp $HOME/TIGERS/files/ASCII-Shadow.flf $PREFIX/share/figlet/
mv $HOME/TIGERS/files/remove /data/data/com.termux/files/usr/bin/
...
echo
    ...
    INPUT_FILE="$HOME/TIGERS/files/.zshrc"
    # Temporary file for output

    # Use sed to replace SIMU with the name and save to a temporary file
    sed "s/SIMU/$name/g" "$INPUT_FILE" > "$HOME/.zshrc"
    sed "s/SIMU/$name/g" "$HOME/TIGERS/files/.tigers.zsh-theme" > "$HOME/.oh-my-zsh/themes/tigers.zsh-theme"
